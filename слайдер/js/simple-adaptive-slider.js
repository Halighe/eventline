...
class ItcSimpleSlider {
  ...
  static EL_STEP = `${ItcSimpleSlider.PREFIX}__step`;
  static EL_STEPS = `${ItcSimpleSlider.PREFIX}__steps`;
  static EL_STEP_ACTIVE = `${ItcSimpleSlider.PREFIX}__step_active`;
  ...
  constructor(target, config) {
    ...
    this._elListStep = document.querySelectorAll(`.${this.constructor.EL_STEP}`);
  ...
  _changeActiveItems() {
    this._states.forEach((item, index) => {
      ...
      if (this._elListStep.length && item) {
        this._elListStep[index].classList.add(this.constructor.EL_STEP_ACTIVE);
      } else if (this._elListStep.length && !item) {
        this._elListStep[index].classList.remove(this.constructor.EL_STEP_ACTIVE);
      }
  ...
  _addIndicators() {
    ...
    for (let i = 0, { length } = this._elListItem; i < length; i++) {
      let steptext = 'Шаг ' + (i+1);
      rowstext += `<li class="${this.constructor.EL_STEP}" data-slide-to="${i}"> ${steptext} </li>`;
    }
    const htmltext = `<ol class="${this.constructor.EL_STEPS}">${rowstext}</ol>`;
    this._el.insertAdjacentHTML('beforeend', htmltext);
